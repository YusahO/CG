...
def read_vertex():
    if last_point[0]:
        sections.append(
            [last_point[:], [int(x_entry.get()), int(y_entry.get())]])
        draw_section(*sections[-1][0], *sections[-1][1], sect_color)
        last_point[0] = None
    else:
...
def get_time():
    ...
    solve()
    mb.showinfo(
        "Время.", f"Время построения: {time.time() - start_time: 8.7f}")
...
def check_polygon():
    ...
    print(verteces_list[0], verteces_list[1], verteces_list[2])
    for i in range(3, len(verteces_list)):
        print(verteces_list[i - 2], verteces_list[i - 1], verteces_list[i])
...
def get_normals_list(verteces):
    ...
    for i in range(length):
        normal_list.append(get_normal(
            verteces[i], verteces[(i + 1) % length], verteces[(i + 2) % length]))
...
def cut(section, verteces_list, normals_list):
    ...
    for i in range(len(verteces_list)):
        if verteces_list[i] != section[0]:
            ...
            wi = get_vect(verteces_list[(i + 1) %
                          len(verteces_list)], section[0])
...
def solve():
    ...
    canvas.create_polygon(*big_list, outline=cutter_color,
                          fill=cfg.CANVAS_COLOUR)
    # CHEAT PART ENDS ===========================================
    print(sections)
...
close_btn = tk.Button(data_frame, text="Замкнуть отсекатель", font=("Consolas", 14),
                      bg=cfg.MAIN_COLOUR, fg=cfg.ADD_COLOUR, command=lambda: return_click(
                          0),
                      activebackground=cfg.ADD_COLOUR, activeforeground=cfg.MAIN_COLOUR)
...
offset += 1
cutter_btn.place(x=0, y=cfg.SLOT_HEIGHT * offset,
                 width=cfg.DATA_WIDTH, height=cfg.SLOT_HEIGHT)
offset += 2
close_btn.place(x=0, y=cfg.SLOT_HEIGHT * offset,
                width=cfg.DATA_WIDTH, height=cfg.SLOT_HEIGHT)
offset += 3

vertex_label.place(x=0, y=cfg.SLOT_HEIGHT * offset,
                   width=cfg.DATA_WIDTH, height=cfg.SLOT_HEIGHT)
offset += 1
...
y_entry.place(x=cfg.DATA_WIDTH // 2, y=cfg.SLOT_HEIGHT * offset,
              width=cfg.DATA_WIDTH // 2, height=cfg.SLOT_HEIGHT)
offset += 1
vertex_btn.place(x=0, y=cfg.SLOT_HEIGHT * offset,
                 width=cfg.DATA_WIDTH, height=cfg.SLOT_HEIGHT)
